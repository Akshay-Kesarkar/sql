...
update bank_details set `default`  = 'NULL' where `default` = 'null' ;
...
DELIMITER &&
create procedure select_pre_filte2(IN var int )
BEGIN
	select * from bank_details where job = 'retired' and balance > var;
END &&

call select_pre_filte2(200);
...
call select_pre_filter3(100 ,'services' );

call select_pre()

select * from (select job , age ,education , y from bank_details) as a  where age =58;
